...
from .filters import CATEGORY_BACKEND, CATEGORY_DATA, CATEGORY_OTHER, CATEGORY_PLATFORM, has_visa_signal, match_score, role_category
...
def build_html(new_jobs: list[Job], active_jobs: list[Job]) -> str:
    def rows(jobs: list[Job]) -> str:
        if not jobs:
            return '<tr><td colspan="9">No matching roles in this category.</td></tr>'
