...
def build_html(new_jobs: list[Job], active_jobs: list[Job]) -> str:
    def rows(jobs: list[Job]) -> str:
        ...
        return "".join(f'<tr><td><a href="{html.escape(job.url, quote=True)}">{html.escape(job.title)}</a></td><td>{html.escape(job.company or "Unknown company")}</td><td>{job.country_tag}</td><td>{"✈️ Visa/Relocation Signal" if has_visa_signal(job) else "⚠️ Verify Sponsorship"}</td><td>{html.escape(analysis_summary(job))}</td><td>{match_score(job):.0%}</td></tr>' for job in jobs)
    ...
    for category in (CATEGORY_DATA, CATEGORY_PYTHON, CATEGORY_PLATFORM):
        ...
        sections.append(f'<section><h2>{html.escape(category)}</h2><h3>🆕 New in the last 24 hours</h3><table border="1" cellpadding="8" cellspacing="0"><tr><th>Role</th><th>Company</th><th>Location</th><th>Visa</th><th>AI analysis</th><th>Fit</th></tr>{rows(category_new)}</table><h3>📋 Active ongoing roles</h3><table border="1" cellpadding="8" cellspacing="0"><tr><th>Role</th><th>Company</th><th>Location</th><th>Visa</th><th>AI analysis</th><th>Fit</th></tr>{rows(category_active)}</table></section>')
