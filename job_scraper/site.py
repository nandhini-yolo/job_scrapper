...
def render(cache: dict) -> str:
    ...
    for category in (CATEGORY_DATA, CATEGORY_PLATFORM, CATEGORY_BACKEND, CATEGORY_OTHER):
        ...
        sections.append(f'<section class="role-section" data-section-category="{html.escape(category, quote=True)}"><h2>{html.escape(category)} <small class="section-count">{len(new_jobs) + len(active_jobs)}</small> roles</h2><h3>New in the last 24 hours <span class="eyebrow">{len(new_jobs)}</span></h3>{cards(new_jobs)}<h3>Active roles <span class="eyebrow">{len(active_jobs)}</span></h3>{cards(active_jobs)}</section>')
