...
def render(cache: dict) -> str:
    ...
    for category in (CATEGORY_DATA, CATEGORY_PLATFORM, CATEGORY_BACKEND, CATEGORY_OTHER):
        ...
        sections.append(f'<section class="role-section" data-section-category="{html.escape(category, quote=True)}"><h2>{html.escape(category)} <small class="section-count">{len(new_jobs) + len(active_jobs)}</small> roles</h2><h3>New in the last 24 hours <span class="eyebrow">{len(new_jobs)}</span></h3><div class="job-list new-list">{cards(new_jobs)}</div><h3>Active roles <span class="eyebrow">{len(active_jobs)}</span></h3><div class="job-list active-list">{cards(active_jobs)}</div></section>')
...
function updateJobs(){{const [search,category,location,difficulty,sort]=controls;const query=search.value.toLowerCase();jobs.forEach(job=>{{const visible=(!query||job.dataset.search.includes(query))&&(!category.value||job.dataset.category===category.value)&&(!location.value||job.dataset.location===location.value)&&(!difficulty.value||job.dataset.difficulty===difficulty.value);job.hidden=!visible;}});document.querySelectorAll('.role-section').forEach(section=>{{const visible=section.querySelectorAll('.job:not([hidden])').length;section.querySelector('.section-count').textContent=visible;let empty=section.querySelector('.filter-empty');if(!visible&&!empty){{empty=document.createElement('p');empty.className='filter-empty empty';empty.textContent='0 roles match the current filters.';section.appendChild(empty);}}if(visible&&empty)empty.remove();}});document.querySelectorAll('.job-list').forEach(list=>{{const listJobs=[...list.querySelectorAll('.job')];listJobs.sort((a,b)=>{{if(sort.value==='company')return a.dataset.company.localeCompare(b.dataset.company);if(sort.value==='role')return a.querySelector('h3').textContent.localeCompare(b.querySelector('h3').textContent);if(sort.value==='difficulty')return Number(a.dataset.difficulty)-Number(b.dataset.difficulty);return b.dataset.firstSeen.localeCompare(a.dataset.firstSeen);}});listJobs.forEach(job=>list.appendChild(job));}});}}
